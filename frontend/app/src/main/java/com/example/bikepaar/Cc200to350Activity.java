...
public class Cc200to350Activity extends AppCompatActivity implements BikeAdapter.OnBikeClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cc200to350);
    ...
    private void fetchBikes() {
        ...
        apiService.get200to350ccBikes(token).enqueue(new Callback<List<Bike>>() {
            @Override
            public void onResponse(Call<List<Bike>> call, Response<List<Bike>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    bikeList = response.body();
                    // Using showBudgetDesign = true for clear prices and images
                    bikeAdapter = new BikeAdapter(Cc200to350Activity.this, bikeList, Cc200to350Activity.this, true);
                    bikesRecyclerView.setAdapter(bikeAdapter);

                    if (bikeList.isEmpty()) {
                        Toast.makeText(Cc200to350Activity.this, "No bikes found in 200-350cc range", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Cc200to350Activity.this, "Failed to load bikes", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Bike>> call, Throwable t) {
                Toast.makeText(Cc200to350Activity.this, "Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e("Cc200to350", "API Error", t);
            }
        });
    }
