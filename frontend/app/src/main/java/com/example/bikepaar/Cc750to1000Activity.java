...
public class Cc750to1000Activity extends AppCompatActivity implements BikeAdapter.OnBikeClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cc750to1000);
    ...
    private void fetchBikes() {
        ...
        apiService.get750to1000ccBikes(token).enqueue(new Callback<List<Bike>>() {
            @Override
            public void onResponse(Call<List<Bike>> call, Response<List<Bike>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    bikeList = response.body();
                    // Using showBudgetDesign = true for clear prices and images
                    bikeAdapter = new BikeAdapter(Cc750to1000Activity.this, bikeList, Cc750to1000Activity.this, true);
                    bikesRecyclerView.setAdapter(bikeAdapter);

                    if (bikeList.isEmpty()) {
                        Toast.makeText(Cc750to1000Activity.this, "No bikes found in 750-1000cc range", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Cc750to1000Activity.this, "Failed to load bikes", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Bike>> call, Throwable t) {
                Toast.makeText(Cc750to1000Activity.this, "Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e("Cc750to1000", "API Error", t);
            }
        });
    }
