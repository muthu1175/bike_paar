...
public class Cc1000AboveActivity extends AppCompatActivity implements BikeAdapter.OnBikeClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cc_above1000);
    ...
    private void fetchBikes() {
        ...
        apiService.getAbove1000ccBikes(token).enqueue(new Callback<List<Bike>>() {
            @Override
            public void onResponse(Call<List<Bike>> call, Response<List<Bike>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    bikeList = response.body();
                    // Using showBudgetDesign = true for clear prices and images
                    bikeAdapter = new BikeAdapter(Cc1000AboveActivity.this, bikeList, Cc1000AboveActivity.this, true);
                    bikesRecyclerView.setAdapter(bikeAdapter);

                    if (bikeList.isEmpty()) {
                        Toast.makeText(Cc1000AboveActivity.this, "No bikes found in 1000cc & Above range", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(Cc1000AboveActivity.this, "Failed to load bikes", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<List<Bike>> call, Throwable t) {
                Toast.makeText(Cc1000AboveActivity.this, "Error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
                Log.e("Cc1000Above", "API Error", t);
            }
        });
    }
